...
import numpy as np
import os
from datasets.utils.skeleton import Skeleton
from datasets.utils.quaternion import *
from datasets.utils.paramUtil import *

import torch
from tqdm import tqdm
from datasets.utils.humanml_utils import HML_JOINT_NAMES, HML_EE_JOINT_NAMES
...
def recover_root_rot_pos(data):
    ...
    if False:
        tmp_pos = torch.zeros(data.shape[:-1] + (3,)).to(data.device)
        tmp_pos[..., 1:, [0, 2]] = data[..., :-1, 1:3]
        tmp_pos = torch.cumsum(tmp_pos, dim=-2)
        # Plot 2D trajectories where each parameter has shape (T, 2)
        fig, ax = plt.subplots(figsize=(24, 24))
        ax.plot(r_pos[..., 0], r_pos[..., 2], 'r', label='recovered')
        ax.plot(tmp_pos[..., 0], tmp_pos[..., 2], 'b', label='original')
        ax.set_xlim(-3.0, 3.0)
        ax.set_ylim(-3.0, 3.0)
        ax.set_aspect('equal')
        ax.legend()
...
def sample_pose_index(data, index):
    rot_vel = data[..., 0]
    r_rot_ang = torch.zeros_like(rot_vel).to(data.device)
    '''Get Y-axis rotation from rotation velocity'''
    r_rot_ang[..., 1:] = rot_vel[..., :-1]
    ...
    ''' recover the dim 0 '''
    r_rot_ang_index = r_rot_ang[..., index]
    r_rot_ang_index[..., 1:] -= r_rot_ang_index[..., :-1].clone()
    new_data_0 = torch.cat([r_rot_ang_index[..., 1:], rot_vel[..., -1:].clone()], dim=-1)
    r_rot_quat_index = r_rot_quat[..., index, :].clone()
    ...
    ''' recover the dim 1 and 2 '''
    r_pos_12 = r_pos[..., index, :]
    r_pos_12[..., 1:, :] -= r_pos_12[..., :-1, :].clone()
    # There is no direct reverse of cumsum, but you can recover the original differences by using torch.diff (or subtraction)
    # For a sequence x, cumsum(x) gives y, and x can be recovered as x = y[..., 1:, :] - y[..., :-1, :]
    # However, here we just want the position at the given index, so we use r_pos_12 as is.
    r_pos_12_ = qrot(r_rot_quat_index, r_pos_12)
    new_data_12 = torch.zeros(r_pos_12_.shape[:-1] + (2,)).to(data.device)
    new_data_12[..., :-1, :] = r_pos_12_[..., 1:, [0, 2]]
    new_data_12[..., -1, :] = data[..., -1, 1:3]

    r_pos[..., 1] = data[..., 3]
    ''' recover the dim 3 '''
    new_data_3 = data[..., index, 3]
    ''' recover the other dims '''
    new_data = torch.cat([new_data_0[..., None], new_data_12, new_data_3[..., None], data[..., index, 4:]], dim=-1)

    if False:
        rot_vel_index = new_data[..., 0]
        rot_vel_ang_index = torch.zeros_like(rot_vel_index).to(new_data.device)
        rot_vel_ang_index[..., 1:] = rot_vel_index[..., :-1]
        rot_vel_ang_index = torch.cumsum(rot_vel_ang_index, dim=-1)
        rot_vel_quat_index = torch.zeros(new_data.shape[:-1] + (4,)).to(new_data.device)
        rot_vel_quat_index[..., 0] = torch.cos(rot_vel_ang_index)
        rot_vel_quat_index[..., 2] = torch.sin(rot_vel_ang_index)

        tmp_pos = torch.zeros(new_data.shape[:-1] + (3,)).to(new_data.device)
        tmp_pos[..., 1:, [0, 2]] = new_data[..., :-1, 1:3]
        tmp_pos = qrot(qinv(rot_vel_quat_index), tmp_pos)
        tmp_pos = torch.cumsum(tmp_pos, dim=-2)
        tmp_pos[..., 1] = new_data[..., 3]

        fig = plt.figure(figsize=(24, 24))
        ax = fig.add_subplot(111, projection='3d')
        ax.scatter(r_pos[..., index, 0], r_pos[..., index, 1], r_pos[..., index, 2], 'r', label='original')
        ax.scatter(tmp_pos[..., 0], tmp_pos[..., 1], tmp_pos[..., 2], 'b', label='sampled')
        ax.set_xlim(-3.0, 3.0)
        ax.set_ylim(-3.0, 3.0)
        ax.set_zlim(-3.0, 3.0)
        ax.set_xlabel('X')
...
def reverse_recover_from_ric(positions, joints_num):
    """
    Reverse of recover_from_ric: 
    Given absolute positions (batch, seq, joints, 3), reconstruct the RIC representation.
    Assumes positions[..., 0, :] is the root joint.
    """
    # positions: (..., joints_num, 3)
    # Output: (..., (joints_num-1)*3 + 4)

    # Extract root position
    r_pos = positions[..., 0, :]  # (..., 3)

    # Compute root rotation (Y-axis) as quaternion
    # Estimate facing direction from hips and shoulders
    # These indices are for HumanML3D: r_hip=2, l_hip=1, sdr_r=17, sdr_l=16
    ...
    across = across1 + across2
    across = across / (torch.norm(across, dim=-1, keepdim=True) + 1e-8)
    up = torch.zeros_like(across)
    up[..., 1] = 1.0
    ...
    # Helper: quaternion between two vectors
    def qbetween(a, b):
        # a, b: (..., 3)
        v = torch.cross(a, b, dim=-1)
        w = (a * b).sum(dim=-1, keepdim=True)
        q = torch.cat([w, v], dim=-1)
        q = q / (torch.norm(q, dim=-1, keepdim=True) + 1e-8)
        return q

    r_rot_quat = qbetween(forward, target)  # (..., 4)

    # Inverse rotate all joints to local root orientation
    from data_loaders.humanml.utils.rotation_conversions import qinv, qrot
    local_joints = positions[..., 1:, :]  # (..., joints_num-1, 3)
    r_rot_quat_expand = r_rot_quat.unsqueeze(-2).expand(local_joints.shape[:-1] + (4,))
    local_joints = qrot(r_rot_quat_expand, local_joints)

    # Subtract root XZ from all joints
    local_joints[..., 0] -= r_pos[..., 0:1]
    local_joints[..., 2] -= r_pos[..., 2:3]

    # Flatten local joints
    local_joints_flat = local_joints.reshape(*local_joints.shape[:-2], -1)  # (..., (joints_num-1)*3)

    # Compose output: [root_y_rot, root_x, root_z, root_y, local_joints]
    # root_y_rot: angle (not quaternion), but in recover_from_ric it's quaternion
    # So, we store quaternion (4), root_x (1), root_z (1), root_y (1), then local joints
    # But recover_from_ric expects: [root_y_rot (1), root_x (1), root_z (1), root_y (1), joints...]
    # But in code, it's [quat (4), root_pos (3), joints...]
    # But in recover_from_ric, data[..., 4:(joints_num-1)*3+4] is joints
    # So, let's match that: [quat (4), root_pos (3), joints...]
    out = torch.cat([r_rot_quat, r_pos], dim=-1)
    out = torch.cat([out, local_joints_flat], dim=-1)
    return out
