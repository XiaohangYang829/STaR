...
def test(
    ...
    penetration_r2et_rate_gt = []
    penetration_mine_rate_gt = []
    penetration_r2et_rate_ret = []
    penetration_mine_rate_ret = []
    penetration_r2et_rate_copy = []
    ...
    for batch_idx, (
        ...
        for i in range(bs):
            ...
            mae.append(1. / height * np.linalg.norm(ret_global - gt_global, ord=2, axis=3).mean())
            mae_local.append(1. / height * np.linalg.norm(ret_local - gt_local, ord=2, axis=2).mean())
            mse.append(1. / height * ((ret_global - gt_global) ** 2).mean())
            mse_local.append(1. / height * ((ret_local - gt_local) ** 2).mean())
            mae_copy.append(1. / height * np.linalg.norm(ret_global_copy - gt_global, ord=2, axis=3).mean())
            mae_local_copy.append(1. / height * np.linalg.norm(ret_local_copy - gt_local, ord=2, axis=2).mean())
    ...
    f.write("Ret MAE_local\t\t" + "{0:.6f}".format(np.mean(mae_local)) + "\n")
    f.write("Copy MAE\t\t" + "{0:.6f}".format(np.mean(mae_copy)) + "\n")
    f.write("Copy MAE_local\t\t" + "{0:.6f}".format(np.mean(mae_local_copy)) + "\n\n")
    ...
    f.write("GT Penetration Rate (R2ET)\t" + "{0:.6f}".format(sum(penetration_r2et_rate_gt) / len(penetration_r2et_rate_gt)) + "\n\n")
    f.write("Ret Penetration Rate (R2ET)\t" + "{0:.6f}".format(sum(penetration_r2et_rate_ret) / len(penetration_r2et_rate_ret)) + "\n\n")
    f.write("Copy Penetration Rate (R2ET)\t" + "{0:.6f}".format(sum(penetration_r2et_rate_copy) / len(penetration_r2et_rate_copy)) + "\n\n")
    f.write("Curvature.\n")
...
def train(
    ...
    epoch_loss_geo = AverageMeter()
    epoch_loss_temp = AverageMeter()
    epoch_loss_vec = AverageMeter()
    ...
    for batch_idx, (
        ...
        globalA_gt = seqA_localnorm[:, :, -4:]
        localA_gt = torch.reshape(seqA_localnorm[:, :, :-4], [bs, T, N, 3]) * train_val_info['local_std'] + train_val_info['local_mean']
        localB_gt = torch.reshape(seqB_localnorm[:, :, :-4], [bs, T, N, 3]) * train_val_info['local_std'] + train_val_info['local_mean']
        quatA_cp = (quatA_norm_cp * train_val_info['quat_std'][None, :] + train_val_info['quat_mean'][None, :])

        param_dict = OrderedDict(
            attention_list = [7, 8, 11, 12, 15, 16, 19, 20],
            num_joints = N,
            temporal_mask = temporal_mask,
            source_height = heightA,
            target_height = heightB,
            all_names = train_val_info['all_names'],
            local_mean = train_val_info['local_mean'],
            local_std = train_val_info['local_std'],
            parents = train_val_info['parents'],
            skelA_norm = skelA_norm,
            skelB_norm = skelB_norm,
            localA_gt = localA_gt,
            quatA_gt = quatA_cp,
            localB_rt = localB_rt,
            localB_base = localB_base,
            localB_gt = localB_gt,
            quatB_rt = quatB_rt,
            quatB_base = quatB_base,
            vecA = vecA,
            vecB_rt = vecB_rt,
            hand_vert_num = arg.hand_vert_num,
            limb_vert_num = arg.limb_vert_num,
            wo_limb_vert_num = arg.wo_limb_vert_num,
            indexesB = indexesB,
            alpha = arg.alpha,
            euler_order = arg.euler_order,
        )
        loss, all_losses_dict = loss_function(arg.loss_args, param_dict, full_mesh_info, sample_mesh_info)
        ...
        epoch_time.update(duration)
        epoch_loss.update(loss.item())
        epoch_loss_geo.update(all_losses_dict['geometric_loss'].item())
        epoch_loss_temp.update(all_losses_dict['temporal_loss'].item())
        epoch_loss_vec.update(all_losses_dict['joint_vector_loss'].item())
        pbar.set_postfix(
            loss=float(loss.item()),
            geo=float(all_losses_dict['geometric_loss'].item()),
            temp=float(all_losses_dict['temporal_loss'].item()),
            vec=float(all_losses_dict['joint_vector_loss'].item()),
            time=duration,
        )
        pbar.update(1)
    scheduler_ret.step()
    ...
    wandb.log({
        ...
        "train_loss_geo": epoch_loss_geo.avg,
        "train_loss_temp": epoch_loss_temp.avg,
        "train_loss_vec": epoch_loss_vec.avg,
        "time": epoch_time.avg,
    })
    ...
    if (epoch + 1) % 2 == 0:
        ...
        curvature_ret_lst = []
        curvature_copy_lst = []
        curvature_source_lst_2 = []
        curvature_ret_lst_2 = []
        curvature_copy_lst_2 = []
        ...
        for batch_idx, (
            ...
            pbar.set_description("Val Epoch %i  Step %i" % (epoch + 1, batch_idx))
            start_time = time.time()
            ...
            for i in range(bs):
                ...
                curv = curvature_1(train_val_info['parents'], skelA[i], quatA[i])
                curvature_source_lst.append(curv)
                curv = curvature_1(train_val_info['parents'], skelB[i], quatB_rt[i])
                curvature_ret_lst.append(curv)
                curv = curvature_1(train_val_info['parents'], skelB[i], quatB_base[i])
                curvature_copy_lst.append(curv)
                curv = curvature_2(train_val_info['parents'], skelA[i], quatA[i])
                curvature_source_lst_2.append(curv)
                curv = curvature_2(train_val_info['parents'], skelB[i], quatB_rt[i])
                curvature_ret_lst_2.append(curv)
                curv = curvature_2(train_val_info['parents'], skelB[i], quatB_base[i])
                curvature_copy_lst_2.append(curv)

            pbar.update(1)

        f = open(os.path.join(arg.work_dir, 'valid-{}.txt'.format(epoch+1)), 'w')
        f.write('valid-{}'.format(epoch+1) + '\n')
        f.write("Penetration.\n")
        f.write("Ret Penetration Rate\t\t" + "{0:.6f}".format(sum(penetration_mine_rate_ret) / len(penetration_mine_rate_ret)) + "\n")
        f.write("Copy Penetration Rate\t\t" + "{0:.6f}".format(sum(penetration_mine_rate_copy) / len(penetration_mine_rate_copy)) + "\n")
        f.write("Ret Penetration Rate (R2ET)\t" + "{0:.6f}".format(sum(penetration_r2et_rate_ret) / len(penetration_r2et_rate_ret)) + "\n")
        ...
        curv_copy = torch.cat(curvature_copy_lst, dim=0).mean(dim=0)
        f.write("Copy Curvature\n\t" + "{}".format(curv_copy) + "\n")
        f.write("Curvature (Vanilla).\n")
        curv_source = torch.cat(curvature_source_lst_2, dim=0).mean(dim=0)
        f.write("Source Curvature\n\t" + "{}".format(curv_source) + "\n")
        curv_ret = torch.cat(curvature_ret_lst_2, dim=0).mean(dim=0)
        f.write("Ret Curvature\n\t" + "{}".format(curv_ret) + "\n")
...
def get_full_mesh_info(scale_info_path, train_feeder, mesh_file_dic):
    ...
    body_bone_lst = train_feeder.body_bone_lst.tolist()[0:4]
    head_bone_lst = train_feeder.body_bone_lst.tolist()[4:]
    leftarm_bone_lst = train_feeder.leftarm_bone_lst.tolist()
    rightarm_bone_lst = train_feeder.rightarm_bone_lst.tolist()
    leftleg_bone_lst = train_feeder.leftleg_bone_lst.tolist()
    rightleg_bone_lst = train_feeder.rightleg_bone_lst.tolist()
    wo_leftarm_bone_lst = train_feeder.wo_leftarm_bone_lst.tolist()
    wo_rightarm_bone_lst = train_feeder.wo_rightarm_bone_lst.tolist()
    wo_leftleg_bone_lst = train_feeder.wo_leftleg_bone_lst.tolist()
    wo_rightleg_bone_lst = train_feeder.wo_rightleg_bone_lst.tolist()
...
def resample_mesh_info(full_mesh_info, mesh_file_dic, arg, sample_replace):
    ...
    for mesh_name in mesh_file_dic.keys():
        ...
        mask = torch.LongTensor(np.random.choice(np.array(range(full_mesh_info['lefthand_vertices'][mesh_name].shape[0])), arg.hand_vert_num, replace=sample_replace)).cuda()
        resample_lefthand_vertices_dic[mesh_name] = full_mesh_info['lefthand_vertices'][mesh_name][mask]
        mask = torch.LongTensor(np.random.choice(np.array(range(full_mesh_info['righthand_vertices'][mesh_name].shape[0])), arg.hand_vert_num, replace=sample_replace)).cuda()
        resample_righthand_vertices_dic[mesh_name] = full_mesh_info['righthand_vertices'][mesh_name][mask]
        # limb
        limb_num = arg.limb_vert_num
        mask = torch.LongTensor(np.random.choice(np.array(range(full_mesh_info['leftarm_vertices'][mesh_name].shape[0])), arg.limb_vert_num, replace=sample_replace)).cuda()
        resample_leftarm_vertices_dic[mesh_name] = full_mesh_info['leftarm_vertices'][mesh_name][mask]
        mask = torch.LongTensor(np.random.choice(np.array(range(full_mesh_info['rightarm_vertices'][mesh_name].shape[0])), arg.limb_vert_num, replace=sample_replace)).cuda()
        resample_rightarm_vertices_dic[mesh_name] = full_mesh_info['rightarm_vertices'][mesh_name][mask]
        mask = torch.LongTensor(np.random.choice(np.array(range(full_mesh_info['leftleg_vertices'][mesh_name].shape[0])), arg.limb_vert_num, replace=sample_replace)).cuda()
        resample_leftleg_vertices_dic[mesh_name] = full_mesh_info['leftleg_vertices'][mesh_name][mask]
        mask = torch.LongTensor(np.random.choice(np.array(range(full_mesh_info['rightleg_vertices'][mesh_name].shape[0])), arg.limb_vert_num, replace=sample_replace)).cuda()
        resample_rightleg_vertices_dic[mesh_name] = full_mesh_info['rightleg_vertices'][mesh_name][mask]
        # wo_limb
        wo_limb_num = arg.wo_limb_vert_num
        mask = torch.LongTensor(np.random.choice(np.array(range(full_mesh_info['wo_leftarm_vertices'][mesh_name].shape[0])), arg.wo_limb_vert_num, replace=sample_replace)).cuda()
        resample_wo_leftarm_vertices_dic[mesh_name] = full_mesh_info['wo_leftarm_vertices'][mesh_name][mask]
        mask = torch.LongTensor(np.random.choice(np.array(range(full_mesh_info['wo_rightarm_vertices'][mesh_name].shape[0])), arg.wo_limb_vert_num, replace=sample_replace)).cuda()
        resample_wo_rightarm_vertices_dic[mesh_name] = full_mesh_info['wo_rightarm_vertices'][mesh_name][mask]
        mask = torch.LongTensor(np.random.choice(np.array(range(full_mesh_info['wo_leftleg_vertices'][mesh_name].shape[0])), arg.wo_limb_vert_num, replace=sample_replace)).cuda()
        resample_wo_leftleg_vertices_dic[mesh_name] = full_mesh_info['wo_leftleg_vertices'][mesh_name][mask]
        mask = torch.LongTensor(np.random.choice(np.array(range(full_mesh_info['wo_rightleg_vertices'][mesh_name].shape[0])), arg.wo_limb_vert_num, replace=sample_replace)).cuda()
...
def main(arg):
    ...
    run = wandb.init(
        project='train STaR',
        entity="neuralbody",
        notes=arg.description,
        tags=arg.wandb_tags,
    )
    wandb.config = {
        "work_dir": arg.work_dir,
        "model_save_name": arg.model_save_name,
        "seed": arg.seed,
        "epochs": arg.epoch,
        "base_learning_rate": arg.base_lr,
        "lr_decay_epoch": arg.step,
        "batch_size": arg.batch_size,
        "weight_decay": arg.weight_decay,
        "loss_args": arg.loss_args,
        "retarget_model": arg.model_path + '.' + arg.model_name,
        "retarget_args": arg.model_args,
        "encoding_model": arg.encoding_model,
        "encoding_model_args": arg.encoding_model_args,
    }

    train_feeder = import_str(arg.train_set)(**arg.train_set_args)
    val_feeder = import_str(arg.train_set)(**arg.train_set_args, is_val=True)
    test_feeder = import_str(arg.test_set)(**arg.test_set_args)
    print('Train Set: {}'.format(len(train_feeder)))
    print('Val Set: {}'.format(len(val_feeder)))
    print('Test Set: {}'.format(len(test_feeder)))
    train_val_info = {
        "global_mean": torch.from_numpy(train_feeder.global_mean).cuda(arg.device[0]),
        "global_std": torch.from_numpy(train_feeder.global_std).cuda(arg.device[0]),
        "local_mean": torch.from_numpy(train_feeder.local_mean).cuda(arg.device[0]),
        "local_std": torch.from_numpy(train_feeder.local_std).cuda(arg.device[0]),
        "quat_mean": torch.from_numpy(train_feeder.quat_mean).cuda(arg.device[0]),
        "quat_std": torch.from_numpy(train_feeder.quat_std).cuda(arg.device[0]),
        "parents": torch.from_numpy(train_feeder.parents).cuda(arg.device[0]),
        "all_names": train_feeder.all_names,
    }
    test_info = {
    ...
    loss_function = import_str(arg.model_path + '.' + arg.loss_function)

    if arg.optimize_param is None:
        optimizer_ret = optim.Adam(retarget_net.parameters(), lr=arg.base_lr, weight_decay=arg.weight_decay, betas=(0.5, 0.999))
    else:
        opt_para_lst = [getattr(retarget_net.module, module).parameters() for module in arg.optimize_param]
        params = []
        for pl in opt_para_lst:
            for p in pl:
                params.append(p)
        print_log_txt(f'Optimize params: ' + str(len(params)), arg.work_dir)
        optimizer_ret = optim.Adam(params, lr=arg.base_lr, weight_decay=arg.weight_decay, betas=(0.5, 0.999))
    scheduler_ret = torch.optim.lr_scheduler.MultiStepLR(optimizer_ret, milestones=arg.step, gamma=0.1, last_epoch=-1)
    ...
    for i in range(arg.epoch):
        ...
        log_txt = (
            'epoch:' + str(i + 1)
            + "  retarget loss:" + str(epoch_loss.avg)
            + "  geometric loss:" + str(epoch_loss_geo.avg)
            + "  temporal loss:" + str(epoch_loss_temp.avg)
            + "  joint vector loss:" + str(epoch_loss_vec.avg)
            + "  epoch time:" + str(epoch_time.avg)
            + "  lr:" + str(lr)
        )
        print_log_txt(log_txt, arg.work_dir)

        if (i + 1) % arg.save_epoch == 0:
            state_dict_ret = retarget_net.state_dict()
            weights_gen = OrderedDict([[k, v.cpu()] for k, v in state_dict_ret.items()])
            torch.save(weights_gen, os.path.join(arg.work_dir, arg.model_save_name + '_final-' + str(i + 1) + '.pt'))
