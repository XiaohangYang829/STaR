...
def q_div_q(a, b):
    # a - b
    b_inv = q_inv(b)
    return q_mul_q(b_inv, a)


def q_inv(a):
    quatA_inv = torch.divide(
        q_neg(a),
        torch.norm(a, dim=3)[:, :, :, None] ** 2,
    )
    return quatA_inv
...
def get_wjs(local_motion, global_motion):
    ...
    rotation = torch.tensor([[[1.0, 0.0, 0.0, 0.0]]], dtype=local_motion.dtype, device=local_motion.device).repeat(local_motion.shape[0], 1, 1)
    translation = torch.tensor([[[0.0, 0.0, 0.0]]], dtype=local_motion.dtype, device=local_motion.device).repeat(local_motion.shape[0], 1, 1)
    axis = torch.tensor([[0.0, 1.0, 0.0]], dtype=local_motion.dtype, device=local_motion.device).repeat(local_motion.shape[0], 1)
    joints_list = []
    ...
    for t in range(int(joints.shape[1])):
        ...
        translation += q_mul_v(
            rotation,
            torch.cat([root_x[:, t : t + 1], root_y[:, t : t + 1], root_z[:, t : t + 1]], dim=-1)[:, None, :],
        )
...
def dir2q(direction):
    # Assuming direction is a tensor of shape (batch_size, ..., 3)
    
    # Calculate the rotation angle (in radians)
    angle = torch.acos(direction[..., 0])  # Assuming x-axis is the first dimension
    
    # Calculate the rotation axis
    axis = torch.nn.functional.normalize(direction[..., 1:], dim=-1)
    
    # Convert the rotation axis to quaternion representation
    half_angle = angle / 2
    quaternion = torch.cat([torch.cos(half_angle).unsqueeze(-1), 
                            torch.sin(half_angle).unsqueeze(-1) * axis], dim=-1)

    return quaternion

def dir2q_(direction):
    x = torch.tensor([1., 0., 0.]).cuda(direction.device)
    theta = torch.acos(direction * x)
    axis = torch.cross(x, direction)
    axis = axis / torch.normalize(axis)
    half_theta = theta / 2
    q = torch.cat([
        torch.cos(half_theta),
        torch.sin(half_theta) * axis
    ], dim=-1)
    return q
