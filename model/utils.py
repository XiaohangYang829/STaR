...
def put_in_world(states):
    ...
    for i in range(len(joints)):
        # not use q_mul_q: checked, already use quaternion multiplication
...
def get_lowerbody_height(joints):
    return (
        np.sqrt(((joints[6, :] - joints[7, :]) ** 2).sum(axis=-1))
        + np.sqrt(((joints[7, :] - joints[8, :]) ** 2).sum(axis=-1))
        + np.sqrt(((joints[8, :] - joints[9, :]) ** 2).sum(axis=-1))
...
def get_distance_from_skel(skel, parents):
    '''
        22*22 matrix, according to parent relationship
    '''

    bs = skel.shape[0]
    diffs = np.sqrt((skel[:, 0]**2).sum(axis=-1))
    batch_distances = []

    for i in range(bs):
        # Initialize an empty adjacency matrix
        n = parents.shape[0]  # Number of nodes
        # adj_matrix = np.zeros((n, n), dtype=int)
        # for child, par in enumerate(parents):
        #     if par != -1:  # Ignore the root node
        #         adj_matrix[par, child] = 1  # Add edge from parent to child
        #         adj_matrix[child, par] = 1
        # adj_matrix[par, child] = 1  # Add edge from parent to child

        adj_matrix = np.zeros((n, n))
        # Fill in the adjacency matrix
        for child, (distance, parent) in enumerate(zip(diffs[i], parents)):
            if parent != -1:  # Ignore the root node
                adj_matrix[parent, child] = distance
                adj_matrix[child, parent] = distance  # Because it's an undirected tree

        # Convert to a compressed sparse row (CSR) matrix for efficiency
        sparse_adj_matrix = csr_matrix(adj_matrix)

        # Run Dijkstra’s algorithm from a source node (e.g., node 0)
        distances = []
        for j in range(22):
            distances.append(np.expand_dims(dijkstra(sparse_adj_matrix, directed=False, indices=j), axis=0))
        distances = np.concatenate(distances, axis=0)
        batch_distances.append(np.expand_dims(distances, axis=0))

    distance_matrix = np.concatenate(batch_distances, axis=0) + 0.01

    return distance_matrix


def get_lowerbody_height_from_skel(skel):
    diffs = np.sqrt((skel**2).sum(axis=-1))
    height = diffs[7:10].sum()
    return height
...
def load_model_w_dis(ret_model, dis_model, arg):
    """ Only apply for networks later than v65 """
    output_device = arg.device[0]

    if arg.ret_weights:
        assert exists(join(arg.work_dir, arg.ret_weights))

        print_log_txt(f'Loading ret weights from {arg.ret_weights}', arg.work_dir)
        ret_weights = torch.load(join(arg.work_dir, arg.ret_weights))
    ...
    if arg.dis_weights and arg.train_with_discriminator:
        assert exists(join(arg.work_dir, arg.dis_weights))

        print_log_txt(f'Loading dis weights from {arg.dis_weights}', arg.work_dir)
        dis_weights = torch.load(join(arg.work_dir, arg.dis_weights))
...
def load_test_model(ret_model, arg):
    output_device = arg.device[0]
    ...
    if arg.test_weights:
        ...
        pop_lst = []
        for k in ret_weights.keys():
            for w in arg.test_ignore_weights:
                if w in k:
                    pop_lst.append(k)
        for k in pop_lst:
            ret_weights.pop(k)
...
def random_point_select(pc, number=1024):
    indexes = np.array(range(pc.shape[0]))
    indexes_select = np.random.choice(indexes, number, replace=False)
    return np.expand_dims(pc[indexes_select], axis=0)


def random_point_select_torch(pc, number=1024):
    indexes = np.array(range(pc.shape[0]))
    indexes_select = torch.LongTensor(np.random.choice(indexes, number, replace=False)).cuda(pc.device)
    return pc[indexes_select].unsqueeze(0)
